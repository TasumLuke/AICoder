// src/components/RGenerator.js
// Lightweight parser: converts a small pseudocode dialect to R,
// and also builds nodes/edges for a flowchart (React Flow compatible).

let uid = 1
const nextId = () => `n${uid++}`

/**
 * parsePseudocodeToR(text)
 * - Input: multiline pseudocode string
 * - Output: string with generated R code
 */
export function parsePseudocodeToR(text) {
  ...
  for (let raw of lines) {
    ...
    // comments
    if (line.startsWith('//')) {
      out.push(`${indent()}# ${line.slice(2).trim()}`)
      continue
    }
    ...
    const ENDIF =
      ...
      /^END IF$/i.test(line) ||
      /^END(IF)?$/i.test(line) ||
      (/ENDIF|END IF|END IF/i.test(line) && /^END(IF)?$/i.test(line))
    ...
    if (IF) {
      ...
      if (stack.length && stack[stack.length - 1] === 'if') {
        // close current indent level for else, but keep flow readable
...
/**
 * buildFlowFromPseudocode(text)
 * - Builds a simple nodes/edges structure compatible with reactflow
 */
export function buildFlowFromPseudocode(text) {
  ...
  function addNode(label) {
    ...
    nodes.push({
      id,
      data: { label },
      position: { x: 40 + nodes.length * 10, y: 40 + nodes.length * 60 },
      style: {
        borderRadius: 14,
        padding: 8,
    ...
    if (IF) {
      ...
      if (top && top.type === 'if') {
        ...
        nodes.push({
          id: afterId,
          data: { label: '⭑ Continue' },
          position: { x: 60 + nodes.length * 6, y: 30 + nodes.length * 40 },
          style: { borderRadius: 12, padding: 6, background: 'linear-gradient(135deg,#fff7f9,#f0f9ff)' }
        })
